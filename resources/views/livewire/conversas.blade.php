    ...
    <section class="flex min-w-0 flex-1 flex-col bg-zinc-50 dark:bg-zinc-950">
        ...
        @else
                ...
                @forelse ($thread as $msg)
                    ...
                    <div @class(['flex', 'justify-start' => $isIn, 'justify-end' => ! $isIn, 'mt-0.5' => $msg['grouped'], 'mt-2' => ! $msg['grouped']])>
                        <div @class([
                            ...
                            @if (!empty($msg['media']))
                                {{-- Prompts 04/05: anexo enviado (rota autenticada/escopada por conta) --}}
                                @if (($msg['media_kind'] ?? 'image') === 'image')
                                ...
                                @else
                                    <a href="{{ $msg['media'] }}" target="_blank" rel="noopener"
                                        ...
                                        <flux:icon icon="document" variant="mini" class="shrink-0 text-red-500" />
            ...
            <div class="shrink-0 border-t border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                @else
                    {{-- Prompts 04/05: erro de validacao do anexo + preview antes de enviar --}}
                    @error('anexo')
